...
public class RequestParams {
	...
	protected final ConcurrentHashMap<String, List<FileWrapper>> fileParams = new ConcurrentHashMap();
	 ...
	 *             throws if wrong File argument was passed
	 */
	public void put(String key, List<File> files, String contentType)
			throws FileNotFoundException {
		if (files == null || files.size() == 0) {
			throw new FileNotFoundException();
		}
		List<FileWrapper> flist = new ArrayList<FileWrapper>();
		for (int i = 0; i < files.size(); i++) {
			if (files.get(i) == null || !files.get(i).exists()) {
				throw new FileNotFoundException();
			}else{
				flist.add(new FileWrapper(files.get(i), contentType));
			}
		}
		if (key != null) {
			fileParams.put(key,flist);
		}
	}
	...
	public String toString() {
		...
		for (ConcurrentHashMap.Entry<String, List<FileWrapper>> entry : fileParams
				.entrySet()) {
			if (result.length() > 0)
				result.append("&");
	...
	private HttpEntity createMultipartEntity(
		...
		for (ConcurrentHashMap.Entry<String, List<FileWrapper>> entry : fileParams
			...
			if(fileWrapperlist!=null&&fileWrapperlist.size()>0){
				...
				for(int i=0;i<fileWrapperlist.size();i++){
					fileWrapper=fileWrapperlist.get(i);
